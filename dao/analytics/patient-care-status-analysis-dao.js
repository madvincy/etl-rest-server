    ...
    return {
        getPatientCareStatusAggregatesQuery: function (queryParams, callback) {
            ...
            if (queryParams.analysis === 'cohortAnalysis') {
                column = "date_format(date('" + queryParams.endDate + "'), '%m/%Y') as reporting_month," +
                    "'" + queryParams.startDate + "' as from_month, " +
                    "'" + queryParams.endDate + "' as to_month, "
            } else {
                column = "date_format(if(to_month,to_month,last_day(date_add(from_month,interval 1 day))), '%m/%Y') as reporting_month," +
            ...
            var where1 = queryParams.analysis === 'cohortAnalysis' ? "t2.endDate = date('" + queryParams.startDate + "')" :
                "t2.endDate >= date('" + queryParams.startDate + "') and t2.endDate <= date('" + queryParams.endDate + "')";
            var where2 = queryParams.analysis === 'cohortAnalysis' ? "t2.endDate = date('" + queryParams.endDate + "')" :
                "t2.endDate >= date('" + queryParams.startDate + "') and t2.endDate <= date('" + queryParams.endDate + "')";
            var onClause =  queryParams.analysis === 'cohortAnalysis' ? "on t1.person_id = t2.person_id ":
            ...
            var sql =
                ...
                "from ( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where1) + " ) `t1` " +


                "left outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where2) + " ) t2 " +
                ...
                "right outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where2) + " ) t2 " +
        ...
        monthlyPatientListQuery: function (queryParams, callback) {
            ...
            var startDate = Moment(Moment(queryParams.startDate).format('YYYY-MM-DD'));
            //var endDate = Moment(Moment(queryParams.endDate).format('YYYY-MM-DD')).endOf('month');
            queryParams.startDate = new Date(startDate.add(-1, 'months')).toISOString().substring(0, 10);
            ...
            var where = " t2.endDate >= date('" + queryParams.startDate + "') and t2.endDate <= date('" + queryParams.endDate + "') ";
            var onClause =" on t1.person_id = t2.person_id and t1.endDate = date_sub(DATE_FORMAT(t2.endDate, '%Y-%m-01'), interval 1 day) ";
            var sql =
                    "select  " + columns.toString() + " from (" +
                    ...
                    "from ( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where) + " ) `t1` " +


                    "left outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where) + " ) t2 " +
                    ...
                    "right outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, where) + " ) t2 " +
                    ...
                    ") t1 " +
                    " INNER JOIN openmrs.person_name `person_name` ON (t1.p1p2 = person_name.person_id and (person_name.voided is null || person_name.voided = 0)) " +
                    "LEFT OUTER JOIN openmrs.patient_identifier `id` ON (t1.p1p2 = id.patient_id) " +
                    "INNER JOIN openmrs.person `person` ON (t1.p1p2 = person.person_id) "
                    + ' group by t1.p1p2 '
        ...
        getPatientCareStatusInternalQuery: function (queryParams, indicators, whereClause) {

            return " SELECT   t2.endDate as reporting_date, " +
