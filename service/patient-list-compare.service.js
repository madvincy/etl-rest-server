    ...
    return new Promise(function (resolve, reject) {
        getAllResultsForHandler(requestObject, handler)
            .then(function (response) {
                ...
                if (compared.summaryStats.totalIncoming !== 0)
                    compared.summaryStats.pocIncomingRatio = (compared.summaryStats.totalPoc / compared.summaryStats.totalIncoming);
...
function resolvePersonIds(personIdsArray) {
    ...
    var sql = "select t1.person_id, t1.uuid as patient_uuid, t4.patient_id, t1.gender," +
        ...
        "as person_name, group_concat(distinct t3.identifier separator ', ') as identifiers FROM openmrs.person `t1`" +
        "INNER JOIN openmrs.person_name `t2` ON (t1.person_id = t2.person_id and (t2.voided is null || t2.voided = 0))" +
        "LEFT OUTER JOIN openmrs.patient `t4` ON (t1.person_id = t4.patient_id)" +
        "LEFT OUTER JOIN openmrs.patient_identifier `t3` ON (t1.person_id = t3.patient_id)" +
